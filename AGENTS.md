...
- Linux Wayland intentionally blocks normal desktop global input APIs. A hardened `cliks-capture` system helper reads evdev and exposes only fixed activity-kind tokens over a target-user-owned `0600` Unix socket. It verifies peer UID and kernel credentials alongside an ephemeral root-generated handshake token stored in restricted runtime storage (`0600`), ignoring environment variable overrides for client binary paths, and emits only while that user owns an active local logind seat. Direct evdev is an explicit compatibility fallback.
- The Linux helper obtains a pidfd for the connecting process (race-free `SO_PEERPIDFD` when the kernel supports it, `pidfd_open` compatibility otherwise), checks liveness, and retains that identity for the connection lifetime. Session switches or helper restarts regenerate the ephemeral token and close existing sockets. Process exit or socket closure immediately removes and closes the authorized client, preventing an inherited socket from outliving the verified Cliks process.
