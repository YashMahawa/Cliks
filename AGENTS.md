...
- Linux Wayland intentionally blocks normal desktop global input APIs. A hardened `cliks-capture` system helper reads evdev and exposes only fixed activity-kind tokens over a target-user-owned `0600` Unix socket. Prior to opening device file descriptors, it inspects udev `ID_SEAT` properties to open only hardware assigned to target user active seats (defaulting untagged devices to `seat0`) and re-evaluates seat rules during each periodic discovery pass. It verifies both peer UID and installed Cliks executable and emits only while that user owns an active local logind seat. Direct evdev is an explicit compatibility fallback.
