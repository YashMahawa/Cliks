...
- Terminal capture owns its saved `term.State`, restores it through deferred/idempotent cleanup, and disables mouse reporting on close or error. On Windows, terminal capture dynamically detects and stores the pre-existing console mode state before disabling QuickEdit mode to prevent application freezes on mouse selection, then restores the original state on session teardown or crash recovery (`repairTerminal`). A raw-mode `Ctrl+C` cancels capture without closing stdin first, so restoration still has a valid terminal file descriptor.
