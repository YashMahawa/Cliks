...
- Bare `cliks`: opens the full-terminal Bubble Tea control screen. The greeting/home view shows selected team name plus a directly clickable code, active local connection state, teammate count, local captured/sent counters, `Open Live`, `Keep Running`, optional `Stop`, `More`, and `Quit`. Normal launches show a centered 2-second animation; the first launch and post-reset launch show a skippable 24-second spatial welcome with bundled sound. Preferences includes Ember/Ocean/Mono themes. Advanced includes Server, which accepts `public` or a self-hosted HTTP(S) URL and derives the WebSocket URL, plus a confirmed device-only Factory Reset that stops local Cliks, disables local autostart, clears local config/state, and restores first-run animations without deleting the server team.
