...
Presence includes an explicit local status: `available`, `focus`, `break`, or `dnd`; unknown values normalize to `available`. Ephemeral reactions are allowlisted to `wave`, `nice`, `coffee`, `focus`, and `celebrate` and are never persisted. Waves target one current same-room peer, have a 30-second per-target cooldown, and are delivered only to sender and recipient. Other reactions are room-wide. The relay accepts at most six reactions per peer per 10 seconds. Background clients may turn an incoming targeted wave into a native notification only when the user enabled notifications; focus and DND suppress it locally, and notification sound has a separate switch.
...
- Bare `cliks`: opens the Bubble Tea control screen. The greeting/home view shows selected team name plus code, active local connection state, teammate count, local captured/sent counters, `Open Live`, `Keep Running`, optional `Stop`, `More`, and `Quit`. A shared footer keeps team, connection, volume/mute, and room count visible across home subviews. Deeper actions live under More: Preferences, Team, Connection, and Diagnostics. Diagnostics renders the same complete structured report as `cliks doctor` in a scrollable in-place view with Back and Refresh controls. Avoid reintroducing a long flat home menu. If a connection is already active, toggling Keep Running off should schedule the active connection to stop when the control screen closes; use the separate Stop action for immediate disconnect.
...
- `cliks start` uses a full-terminal Bubble Tea spatial desk when stdin/stdout are TTYs. The listener sits in the center; up to 12 named peers occupy adaptive depth rings, current typers light up, new peers animate in, and larger rooms collapse overflow into semantic dots. The first live visit shows a short synthetic welcome arrangement without sending activity. A side panel shows typing, people, flow, health, capture, listening levels, presence, notification state, selected peer, and recent ephemeral reactions. Controls add `j/k` peer selection, `1` wave, `2` nice, `3` coffee, `4` celebrate, and `p` presence cycling alongside Up/Down volume, Left/Right or `[`/`]` density, `m` mute, `s` spatial, and `f` fatigue fade. `Tab` or `Shift+S` opens unified preferences without disconnecting. `?` opens context-specific shortcuts. From live, `Esc`, `q`, or Back returns to the main control screen; Stop or `Ctrl+C` disconnects. Keep Running hands a closing foreground room to one background session. SIGHUP/SIGTERM must cancel Bubble Tea through context; never call `os.Exit` from TUI/session signal handlers. Non-TTY runs fall back to plain status output, and semantic colors must use light/dark adaptive pairs.
