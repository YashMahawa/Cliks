...
Teams track the last successful live connection. Only servers with `CLIKS_EXPIRE_INACTIVE_TEAMS=true` automatically soft-delete after 48 hours without a connection; the public Render service enables it, and self-hosted servers default to no expiry. Successful joins refresh the deadline. The hourly cleanup refreshes rooms that are currently occupied before expiring inactive rows, so a continuously connected room stays alive. New store implementations must preserve this lifecycle and the join/expiry race protection.
...
The website exposes a password-confirmed Delete room form, uses the Render public relay unless a custom URL is configured, waits up to 75 seconds for a sleeping relay, and shows a short local cooldown after room actions. The TUI also guards rapid repeated room submissions. These client guards complement, but never replace, the server's per-IP create/delete/lookup/join rate limits.

Treat team codes returned by every backend, including self-hosted backends, as untrusted input. The CLI accepts only `CLIK-` plus six ASCII alphanumeric characters (and the explicit local-test code `CLIK-LOCAL`) before using a code in a URL, saved config, systemd unit, LaunchAgent, or Windows Startup script. Launcher executable paths must reject control characters; systemd `%` specifiers must be escaped and LaunchAgent XML text must be encoded.
...
Personal room tones are separate from teammate activity samples. `off`, `still`, `lofi`, `cloud`, `contemplation`, `downtempo`, `rain`, `fire`, and `cafe` select eight CC0 tracks embedded as normalized MP3 assets and never synchronized or sent. Onboarding previews each option for up to six seconds and cancels the previous preview when focus moves. Their level is adjustable from 5-100%; selecting `off` is the zero-volume state. macOS and Windows decode and loop them through the built-in Oto context. Linux/Termux decode them to a versioned cached WAV, then use the first available mpv, ffplay, PulseAudio, PipeWire, ALSA, or Termux player. ALSA and Termux players have no reliable per-process volume flag, so Cliks applies gain to a temporary private WAV before looping it. Muting Cliks also pauses the room tone. Source and license details live in `cli/assets/ambient/NOTICE.md`.
...
- `cliks join CODE`: validates and saves the team, then starts one detached background session by default. If another team owns the local session, Cliks stops it and waits for it to exit before starting the newly joined team; two teams can never stay connected from one local state directory. `cliks join --no-start CODE` only saves/selects the team for users who explicitly want the old manual flow.
...
- `cliks config`: prints saved configuration plus current launch-at-login state. `cliks set` accepts one or more exact key/value pairs, validates the entire batch, and performs one atomic save; values containing spaces must be shell-quoted. Invalid booleans are errors instead of silently becoming `off`. `cliks set autostart on|off` is a separate side-effecting operation. `cliks set audio.device DEVICE|default` configures advanced output routing only where the selected player supports it. API/WebSocket endpoint changes tell an active local owner to reconnect with the saved endpoint instead of leaving a stale daemon. `CLIKS_API_URL` and `CLIKS_WS_URL` override saved URLs at load time.
...
- `cliks background start|stop|status [team-code]`: runs `cliks start` detached from the terminal for the selected team, reports the pid/log path/session state, or stops it. Start waits up to two seconds for the child to own a matching session lock and remain stable briefly; an early exit/timeout is reported with recent log output instead of a false success. Use this for "close the terminal but keep Cliks connected" behavior; `cliks autostart` is for login-time launch.
...
- `cli/install.sh`: installs a matching native release plus Cliks Capture.app on macOS or the hardened system helper on Linux, then runs `cliks setup`. Optional Linux/Termux audio, clipboard, and notification package failures (including a busy package-manager lock) must warn and continue so an otherwise buildable/installable Cliks binary still replaces the old version. It must never automatically grant the desktop user raw input ACL/group access. `cli/install.ps1` remains the native Windows installer. Termux never receives desktop-wide capture claims or sudo setup.
...
- The Linux helper obtains a pidfd for the connecting process (race-free `SO_PEERPIDFD` when the kernel supports it, `pidfd_open` compatibility otherwise), checks liveness before and after executable verification, and retains that identity for the connection lifetime. Process exit or socket closure immediately removes and closes the authorized client, preventing an inherited socket from outliving the verified Cliks process.
...
- Terminal-mode reads use a platform-aware cancelable reader. Stop/session cancellation must interrupt a blocked stdin read and close its helper resources without closing process-wide stdin.
...
- Choosing Stop in an interactive Live session disconnects and returns to the home control screen, including when Live was opened with `cliks start` or `cliks live`. Quit App remains the explicit terminal exit. Attached background sessions stop through the same path.
...
The current public backend is `https://cliks-server.onrender.com` (Render service `srv-dapuomou01pc73e1knb0`, free Postgres `dpg-dapuoeegekts73f9of0g-a`). Render's free web service sleeps after 15 idle minutes; waking can take about a minute. Its free Postgres expires on October 23, 2026. Migrate or upgrade before then. Active team rows from the former DigitalOcean Droplet were imported into Render with codes and delete password hashes intact before the Droplet was destroyed. `deploy/droplet-cloud-init.yaml` remains a self-hosting option.
...
`cli/config.go` points new installs at `https://cliks-server.onrender.com` and migrates saved configurations whenever both API and WebSocket endpoints exactly match the old Droplet default, including devices with saved teams. It preserves saved team history and persists the new endpoints; custom endpoints are never rewritten, and environment overrides still take precedence. The old active room rows were migrated before Droplet destruction to avoid unavailable-code cleanup. This is a public backend URL, not a secret. Never put Render, DigitalOcean, database, or service credentials into the CLI, website bundle, README, install script, or committed env files.
