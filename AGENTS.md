...
- `cliks config`: prints saved configuration plus current launch-at-login state. `cliks set` accepts one or more exact key/value pairs, validates the entire batch, and performs one atomic save; values containing spaces must be shell-quoted. Invalid booleans are errors instead of silently becoming `off`. `cliks set autostart on|off` is a separate side-effecting operation. `cliks set audio.device DEVICE|default` configures advanced output routing only where the selected player supports it. Proactive endpoint validation probes the target output device during startup, setting changes, and sound execution, issuing a single warning message and automatically reverting to default system audio when an endpoint is unreachable or invalid. API/WebSocket endpoint changes tell an active local owner to reconnect with the saved endpoint instead of leaving a stale daemon. `CLIKS_API_URL` and `CLIKS_WS_URL` override saved URLs at load time.
