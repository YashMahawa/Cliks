...
- `cliks doctor`: explains privacy, builds the shared structured Go/audio/input-device report, executes dynamic capability probing across capture and audio providers via `ProbeHardwareSubsystem`, and prints detected fix commands. Joining remains non-blocking but surfaces a short passive note when that report finds audio or capture blockers. Unified `InputCaptureProvider` and `AudioBackendProvider` contracts support dynamic initialization probing and automated runtime fallbacks across macOS, Windows, and Linux.
