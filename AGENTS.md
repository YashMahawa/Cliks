...
- `cliks join CODE`: validates and saves the team, then starts one detached background session by default. Supports `-f` or `--force` to bypass active session prompts. If an active session is running, interactive terminal users are prompted for confirmation before disconnecting, while non-interactive shells exit with an error unless `--force` is passed. If another team owns the local session and join is confirmed or forced, Cliks stops it and waits for it to exit before starting the newly joined team; two teams can never stay connected from one local state directory. `cliks join --no-start CODE` only saves/selects the team for users who explicitly want the old manual flow.
