...
- `cli/install.sh`: hassle-free installer — builds CLI, installs **mpv** for spatial sound, auto PATH export, Linux input ACL/group when possible, opens macOS Accessibility settings, ends with `cliks setup`. Non-interactive when piped (`CLIKS_AUTO_YES=1`). Termux is allowed as a non-supported test shell and must not be sent through sudo/input-group setup. Keep this user-facing and never request or print backend provider tokens.
