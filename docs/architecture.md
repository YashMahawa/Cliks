...
The separate social surface stays intentionally constrained. Presence carries an explicit four-state status (`available`, `focus`, `break`, `dnd`). Ephemeral reactions use the fixed `wave`, `nice`, `coffee`, `celebrate`, `break`, and `focus` allowlist and are not stored. A wave targets one same-room peer, is private to sender/recipient, and has a 30-second per-target cooldown; room reactions and waves share a six-per-10-second sender limit. Enabled clients may show incoming signals through native macOS Notification Center, Windows toast, Linux `notify-send`, or Termux notifications. Every signal maps to a fixed emoji and phrase, so modified clients cannot send arbitrary notification text. Focus/DND suppress delivery locally and notification sound is independently configurable. Linux background delivery reconstructs the current user's D-Bus address when launch environments omit it.
...
The Go CLI uses Bubble Tea for a full-terminal spatial desk and settings UI. The desk centers the listener, draws adaptive depth rings, labels up to 12 peers, marks current typers, animates arrivals and reactions at the sending peer's seat, and uses compact overflow dots as rooms grow. A three-second normal launch and ten-second first launch both use bundled sound; first launch then opens an optional guided setup for nickname, OS permissions, notification delivery, preferences, and server selection. Notification titles contain sender, glyph, and fixed phrase because some desktop shells collapse body text. The first-live-run synthetic arrangement teaches the map without creating network events. The direct action rail derives exact mouse hitboxes from rendered display widths, avoiding stale or shifted coordinates. Interactive listening and presence controls persist locally:
