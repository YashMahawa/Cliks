...
The separate social surface stays intentionally constrained. Presence carries an explicit four-state status (`available`, `focus`, `break`, `dnd`). Ephemeral reactions use the fixed `wave`, `nice`, `coffee`, `celebrate`, `break`, and `focus` allowlist and are not stored. Every reaction is room-wide, including Wave; legacy target fields are ignored. Room signals share a six-per-10-second sender limit. Enabled clients may show incoming signals through native macOS Notification Center, Windows toast, Linux `notify-send`, or Termux notifications. Every signal maps to a fixed emoji and phrase, so modified clients cannot send arbitrary notification text. Local mute suppresses remote reaction animation and notification delivery, focus/DND suppress native notifications, and notification sound is independently configurable. Linux background delivery reconstructs the current user's D-Bus address when launch environments omit it.
